...
def calculate(x, y, id2word, id2tag, res=None):
    if res is None:
        res = []
    entity = []
    for j in range(len(x)):
        if x[j] == 0 or int(y[j]) == 0:
            continue
        if id2tag[int(y[j])][0] == 'B':
            entity = [id2word[int(x[j])] + '/' + id2tag[int(y[j])]]
        elif id2tag[int(y[j])][0] == 'M' and len(entity) != 0 and entity[-1].split('/')[1][1:] == id2tag[int(y[j])][1:]:
            entity.append(id2word[int(x[j])] + '/' + id2tag[int(y[j])])
        elif id2tag[int(y[j])][0] == 'E' and len(entity) != 0 and entity[-1].split('/')[1][1:] == id2tag[int(y[j])][1:]:
            entity.append(id2word[int(x[j])] + '/' + id2tag[int(y[j])])
            entity.append(str(j))
            res.append(entity)
            entity = []
...
def calculate3(x, y, id2word, id2tag, res=None):
    """
